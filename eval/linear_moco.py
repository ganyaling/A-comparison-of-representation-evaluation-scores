...
parser.add_argument('--batch_size', default=34, type=int,
                    help='mini-batch size for training')
parser.add_argument('--val_batch_size', default=48, type=int,
...
class MoCo_model(nn.Module):
    ...
    def build_model(self):
        # Build model with architecture from checkpoint
...
def flatten_feats(feats):
    # ResNet-like 输出 (B,C,H,W) -> GAP -> (B,C)
    if feats.ndim == 4:
...
def main():
    ...
    backbone = MoCo_model(checkpoint_path=args.checkpoint, device=device)
    backbone_model = backbone.build_model()

    # Create save directory structure: saved_features/linear_moco/{dataset}/{checkpoint_name}/
    checkpoint_name = os.path.splitext(os.path.basename(args.checkpoint))[0]
    save_dir = os.path.join('saved_features', 'linear_moco', args.dataset, checkpoint_name)
